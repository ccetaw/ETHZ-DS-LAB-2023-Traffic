...
# Read the gis data
path_to_canton = "./spatial/CANTONloops"
path_to_astra = "./spatial/ASTRAloops/"
path_to_dav = "./spatial/DAVloops/"
...
# dav_gdf = geopandas.read_file(path_to_dav).to_crs("EPSG:4326")
# loops_gdf = geopandas.read_file(path_to_loops).to_crs("EPSG:4326")


# Read the sensor statistics
canton_sensor_df = pd.read_csv("./data/reducted.csv", index_col=['sensor_id', 'date'])


# Draw a figure for each sensor
for sensor_id in canton_gdf['detid']:
    fig=make_subplots(specs=[[{"secondary_y":True}]])

    # Draw #cars per day per sensor
    fig.add_trace(                           #Add a bar chart to the figure
            go.Bar(
            x=canton_sensor_df['count'][2].keys(),
            y=canton_sensor_df['count'][2].values,
            name="#Cars per day",
    ...
    fig.write_html("./figs/"+sensor_id+".html")
...
# dav_geo_list = np.array([[point.xy[1][0], point.xy[0][0]] for point in dav_gdf['geometry']])
...
# dav_sensors = folium.FeatureGroup(name="dav_sensors")
# loops_sensors = folium.FeatureGroup(name="loops_sensors")

for index, row in canton_gdf.iterrows():
    html="""
    <iframe src=\"""" + "./figs/" + row['detid'] + ".html" + """\"width="850" height="400"  frameborder="0">    
    """
    popup = folium.Popup(folium.Html(html, script=True))
    folium.CircleMarker(location=canton_geo_list[index], radius=5, color="#3388ff", fill=True, popup=popup).add_to(canton_sensors)

for sensor_position in astra_geo_list:
    folium.CircleMarker(location=sensor_position, radius=5, color="crimson", fill=True).add_to(astra_sensors)

# for sensor_position in canton_geo_list:
#     folium.Marker(location=sensor_position, icon=folium.Icon(color="orange")).add_to(dav_sensors)
